...
case "$AGENT_TYPE" in
    ...
    "copilot")
        ...
        ;;
    "cursor")
        update_agent_file "$CURSOR_FILE" "Cursor AI"
        ;;
    "")
        ...
        [ -f "$CURSOR_FILE" ] && update_agent_file "$CURSOR_FILE" "Cursor AI"
        ...
        if [ ! -f "$CLAUDE_FILE" ] && [ ! -f "$GEMINI_FILE" ] && [ ! -f "$COPILOT_FILE" ] && [ ! -f "$CURSOR_FILE" ]; then
            echo "No agent context files found. Creating Cursor AI context file by default."
            update_agent_file "$CURSOR_FILE" "Cursor AI"
        fi
        ;;
    *)
